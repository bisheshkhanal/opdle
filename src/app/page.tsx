    ...
    <main className="flex min-h-screen flex-col">
      ...
      <header className="border-b border-parchment-300/40 bg-gradient-to-b from-parchment-50/95 via-parchment-100/90 to-parchment-100/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl flex-col items-center px-4 py-8 sm:py-10">
          <div className="mb-3 inline-flex items-center gap-3 rounded-full border border-parchment-300/60 bg-parchment-50/70 px-4 py-1.5 font-display text-[11px] uppercase tracking-[0.3em] text-navy-600 shadow-soft">
            Treasure Log
          </div>
          <h1 className="mb-3 font-display text-4xl font-semibold tracking-tight text-navy-800 sm:text-5xl">
            <span className="text-tile-wrong">One</span>
            <span className="text-navy-700">Piece</span>
            <span className="text-gold-600">dle</span>
          </h1>
          <div className="mb-4 h-0.5 w-32 rounded-full bg-gradient-to-r from-gold-400/80 via-gold-300/40 to-transparent" />
          <p className="mb-6 text-sm text-navy-500 sm:text-[15px]">
            Guess the character in {MAX_GUESSES} tries
            {mode === "daily" && (
              <span className="ml-2 inline-flex items-center rounded-full bg-navy-100/70 px-2.5 py-0.5 text-xs font-medium text-navy-700 ring-1 ring-navy-200/50">
                #{getDailyGameNumber()}
              </span>
            )}
          </p>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_280px]">
            ...
            <div className="w-full max-w-5xl">
              <div className="game-stage">
                ...
                <svg
                  className="stage-compass text-navy-700"
                  viewBox="0 0 200 200"
                  aria-hidden="true"
                >
                  <circle
                ...
                {!isFinished && (
                  <div className="relative !z-20 mb-8 flex flex-col items-center gap-4">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3">
                      <span className="rounded-full border border-parchment-300/70 bg-parchment-100/70 px-3 py-1 text-[12px] font-medium text-navy-600">
                        Guess {guesses.length + 1} of {MAX_GUESSES}
                      </span>
                      {duplicateWarning && (
                        <span
                          className="animate-scale-in rounded-full bg-gold-100/80 px-3 py-1.5 text-sm font-medium text-gold-800 ring-1 ring-gold-200/50"
                          role="alert"
                        >
                          {duplicateWarning}
                        </span>
                  ...
                  <div className="flex flex-col items-center gap-6">
                    ...
                    {mode === "daily" && (
                      <div className="game-card px-6 py-5 text-center">
                        ...
                        <p className="mt-1.5 font-mono text-2xl font-bold tracking-tight text-navy-800">
                ...
                {guesses.length === 0 && !isFinished && (
                  <div className="game-card mx-auto max-w-lg p-7 text-center sm:p-8">
                    <div className="mb-5 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-navy-100/70 ring-1 ring-navy-200/40">
                      <svg
                    ...
                    <h2 className="mb-2.5 font-display text-xl font-semibold tracking-tight text-navy-800 sm:text-2xl">
                      Start typing a character name
                    </h2>
                    <p className="text-sm text-navy-500 sm:text-[15px]">
                      Each guess reveals clues about the mystery character
                    </p>
                    <div className="mt-6 flex flex-wrap justify-center gap-2">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-tile-correct/10 px-3 py-1.5 text-xs font-medium text-tile-correct ring-1 ring-tile-correct/20">
                        <span className="h-2 w-2 rounded-full bg-tile-correct" />
                        Correct
                      </span>
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-tile-partial/10 px-3 py-1.5 text-xs font-medium text-tile-partial ring-1 ring-tile-partial/20">
                        <span className="h-2 w-2 rounded-full bg-tile-partial" />
                        Partial
                      </span>
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-tile-wrong/10 px-3 py-1.5 text-xs font-medium text-tile-wrong ring-1 ring-tile-wrong/20">
                        <span className="h-2 w-2 rounded-full bg-tile-wrong" />
                        Wrong
                      </span>
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-navy-100/70 px-3 py-1.5 text-xs font-medium text-navy-600 ring-1 ring-navy-200/40">
                        <span>↑↓</span>
                        Higher/Lower
                      </span>
                    </div>
      ...
      {/* Footer */}
      <footer className="border-t border-parchment-300/40 bg-gradient-to-t from-parchment-100/95 via-parchment-100/90 to-parchment-50/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl flex-col items-center gap-2.5 px-4 py-6 text-center text-sm text-navy-500 sm:flex-row sm:justify-between">
          <Link
            href="/about"
            className="font-medium text-navy-600 underline-offset-2 transition-all hover:text-navy-800 hover:underline"
          >
            About / How to Play
          </Link>
