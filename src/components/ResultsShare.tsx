...
export function ResultsShare({
  ...
  const [textShareStatus, setTextShareStatus] = useState<"idle" | "success">("idle");
  ...
  const handleShareCard = async () => {
    ...
    try {
      ...
      const file = new File([blob], "onepiecedle-result.png", { type: "image/png" });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({
            title: "Onepiecedle Result",
            text: formatShareText(guesses, mode, isWon, dateString, streak, hintUsed, ruleset),
            files: [file],
          });
